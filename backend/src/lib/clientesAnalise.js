...
function calcularBaseRFV() {
  ...
  const resultado = comHistorico.map(x => {
    ...
    return {
      ...
      recompensas_disponiveis: Math.floor((x.cliente.total_pedidos + (x.cliente.selos_bonus || 0)) / 10) - x.cliente.recompensas_usadas,
  ...
  for (const c of semHistorico) {
    resultado.push({
      ...
      recompensas_disponiveis: Math.floor((c.total_pedidos + (c.selos_bonus || 0)) / 10) - c.recompensas_usadas,
